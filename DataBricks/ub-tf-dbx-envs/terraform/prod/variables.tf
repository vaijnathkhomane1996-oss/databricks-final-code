...
variable "shared_metastore_id" {
  type        = string
  description = "ID of the shared Unity Catalog metastore (created in intg, used by stag/prod)"
  default     = null
}

variable "create_metastore" {
  type        = bool
  description = "Whether to create a new metastore (true for intg, false for stag/prod to use shared)"
  default     = true
}
