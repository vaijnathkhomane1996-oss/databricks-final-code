locals {
  ...
  # Auto-compute metastore name (references existing shared metastore)
  # Note: This is for reference only - actual metastore is provided via shared_metastore_id
  uc_metastore_name = try(var.workspace.uc_metastore_name, "${var.product_name}-shared-metastore-${var.region}")
