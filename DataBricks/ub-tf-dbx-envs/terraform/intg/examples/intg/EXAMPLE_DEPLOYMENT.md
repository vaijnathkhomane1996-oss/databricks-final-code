# Integration Environment - Complete Deployment Guide

This guide provides step-by-step instructions to deploy the Integration Databricks environment. **⚠️ IMPORTANT: Integration environment MUST be deployed AFTER staging environment** because it uses the staging metastore.
...
**⚠️ CRITICAL:** Integration environment uses the staging metastore. You MUST deploy staging first.
...
```hcl
workspace = {
  workspace_name = "dp-intg-ws-us-east-2"  # Format: {product}-{env}-ws-{region}
  pricing_tier   = "STANDARD"              # STANDARD, PREMIUM, or ENTERPRISE
  
  # Unity Catalog Metastore Configuration
  # IMPORTANT: Integration uses SHARED metastore from staging
  ...
  clusters = {
    cl1 = {
      cluster_name  = "dp-intg-cluster-a"  # Your cluster name
  ...
  # IMPORTANT: Use unique catalog names (intg_catalog1, intg_catalog2, intg_catalog3)
  # to avoid conflicts with staging catalogs in the shared metastore
  catalogs = {
    intg_catalog1 = {
      storage_root = "s3://dp-shared-uc/intg/catalog1/"  # Your S3 path
      grants       = []                                   # Optional grants
    }
    intg_catalog2 = {
      storage_root = "s3://dp-shared-uc/intg/catalog2/"
      grants = [
        {
    ...
    intg_catalog3 = {
      storage_root = "s3://dp-shared-uc/intg/catalog3/"
      grants = [
        {
...
```hcl
# IMPORTANT: Integration environment uses staging metastore
...
This pass creates the S3 bucket, Databricks workspace, and assigns the staging metastore to the integration workspace.
...
- Secret name: `damage-prevention-intg-us-east-2-databricks-workspace`
- Staging metastore was assigned to integration workspace
...
- This confirms the integration workspace is using the shared staging metastore
...
3. **Generate new token:**
   ...
   - Add a comment: `Terraform automation for integration`
   - Set lifetime:
...
**Expected output:**
```
Secret damage-prevention-intg-us-east-2-databricks-workspace already exists. Updating...
...
```bash
SECRET_NAME="damage-prevention-intg-us-east-2-databricks-workspace"
WORKSPACE_URL="https://dbc-xyz12345-6789.cloud.databricks.com"
...
1. **Go to AWS Secrets Manager:**
   ...
   - Find secret: `damage-prevention-intg-us-east-2-databricks-workspace`
...
aws secretsmanager get-secret-value \
  --secret-id "damage-prevention-intg-us-east-2-databricks-workspace" \
  --region us-east-2 \
  --query SecretString \
...
- Should see: `intg_catalog1`, `intg_catalog2`, `intg_catalog3`
...
3. **Verify catalogs:**
   - Go to **Data** → **Catalogs**
   - You should see **6 catalogs total** (3 from staging + 3 from integration):
     - **Staging catalogs:** `catalog1`, `catalog2`, `catalog3`
     - **Integration catalogs:** `intg_catalog1`, `intg_catalog2`, `intg_catalog3`
   - **Note:** All 6 catalogs are in the same metastore (staging metastore)
...
4. **Verify metastore:**
   ...
   - This is the **same metastore** used by staging workspace
...
1. **S3 Bucket:**
   ```bash
   aws s3 ls | grep damage-prevention-intg
   ```
   Should show: `dp-damage-prevention-intg-us-east-2-s3`
...
**Verify both workspaces use the same metastore:**
...
# Get integration metastore ID
cd ../intg
INTG_METASTORE_ID=$(terraform output -raw metastore_id)
echo "Integration Metastore ID: $INTG_METASTORE_ID"

# They should match!
if [ "$STAGING_METASTORE_ID" = "$INTG_METASTORE_ID" ]; then
...
2. Get staging metastore ID:
   ...
   ```
3. Update integration `terraform.tfvars`:
   ```hcl
...
**Solution:**
- Use unique catalog names in integration
- Current configuration uses: `intg_catalog1`, `intg_catalog2`, `intg_catalog3`
...
2. Verify metastore ID is correct in integration `terraform.tfvars`

3. Ensure integration workspace is created first (Pass-1)
...
1. Verify catalog names are unique:
   - Integration should use: `intg_catalog1`, `intg_catalog2`, `intg_catalog3`
...
3. If conflict exists:
   - Update integration `terraform.tfvars` with different catalog names
   - Re-run `terraform apply`
...
2. ✅ Get staging metastore ID
3. ✅ Update integration `terraform.tfvars` with staging metastore ID
4. ✅ Deploy integration environment (Pass-1)
5. ✅ Create and store PAT
6. ✅ Deploy integration environment (Pass-2)
...
- `terraform.tfvars` - Your configuration (copy from `examples/intg/terraform.tfvars.example`)

### Important Variables

- `shared_metastore_id` - Staging metastore ID (from staging outputs) ⭐
- `create_metastore = false` - Use shared metastore ⭐
- `workspace.catalogs` - Use unique catalog names (e.g., `intg_catalog1`) ⭐
...
- **Integration README**: [`../README.md`](../README.md)
