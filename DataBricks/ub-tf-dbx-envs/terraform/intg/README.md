# Integration Environment - Databricks Infrastructure

This directory contains the Terraform configuration for deploying the **Integration** Databricks environment using Repo B (`ub-tf-dbx-platform`).
...
The integration environment creates a complete Databricks platform with:
- **1 Databricks Workspace** (`dp-intg-ws-us-east-2`)
- **1 Unity Catalog Metastore** (`dp-stag-metastore-us-east-2`) - **Shared from staging, assigned to integration workspace**
- **3 Compute Clusters** (cl1, cl2, cl3)
- **3 Unity Catalog Catalogs** (intg_catalog1, intg_catalog2, intg_catalog3)
...
- **Naming Convention**: All resources include region suffix (e.g., `dp-intg-ws-us-east-2`)
...
│                    (ub-tf-dbx-envs/terraform/intg)                          │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Repo C: Integration Environment Module                    │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐  │
│  │  main.tf                                                             │  │
│  │  └─ module "dbx_platform" {                                         │  │
...
│  │                                                                      │  │
│  │  1. S3 Bucket                                                        │  │
│  │     └─ dp-damage-prevention-intg-us-east-2-s3                        │  │
│  │                                                                      │  │
│  │  2. Databricks Workspace                                             │  │
│  │     └─ dp-intg-ws-us-east-2                                         │  │
│  │        ├─ VPC: vpc-xxx                                               │  │
...
│  │        └─ Assigned to: dp-intg-ws-us-east-2                         │  │
│  │                                                                      │  │
│  │  4. Compute Clusters (3 clusters)                                  │  │
│  │     ├─ dp-intg-cluster-a (i3.xlarge, 2 workers)                     │  │
│  │     ├─ dp-intg-cluster-b (i3.2xlarge, 3 workers)                    │  │
│  │     └─ dp-intg-cluster-c (m5.xlarge, 4 workers)                     │  │
│  │                                                                      │  │
│  │  5. Unity Catalog Catalogs (3 catalogs)                             │  │
│  │     ├─ intg_catalog1 → s3://damage-prevention-dashboard-intg-us-east-2/catalogs/intg/catalog1/  │  │
│  │     │   └─ Metastore: dp-stag-metastore-us-east-2 ⭐ SHARED        │  │
│  │     ├─ intg_catalog2 → s3://damage-prevention-dashboard-intg-us-east-2/catalogs/intg/catalog2/  │  │
│  │     │   └─ Metastore: dp-stag-metastore-us-east-2 ⭐ SHARED         │  │
│  │     └─ intg_catalog3 → s3://damage-prevention-dashboard-intg-us-east-2/catalogs/intg/catalog3/  │  │
│  │         └─ Metastore: dp-stag-metastore-us-east-2 ⭐ SHARED         │  │
...
│  │  └─ Secret: dp-damage-prevention-intg-us-east-2-databricks-       │  │
│  │             workspace                                               │  │
│  │     ├─ workspace_id                                                 │  │
│  │     ├─ workspace_url                                                │  │
...
6. 3 Unity Catalog Catalogs Created
   ├─ intg_catalog1 (metastore: dp-stag-metastore-us-east-2) ⭐
   ├─ intg_catalog2 (metastore: dp-stag-metastore-us-east-2) ⭐
   └─ intg_catalog3 (metastore: dp-stag-metastore-us-east-2) ⭐
   Note: Catalogs created without schemas
...
### 1. **S3 Bucket**
- **Name**: `dp-damage-prevention-intg-us-east-2-s3` (format: `{prefix}-{product_name}-{env}-{region}-s3`)
- **Purpose**: Databricks artifacts, logs
- **Features**: Versioning enabled, tagged with corporate tags

### 2. **Databricks Workspace**
- **Name**: `dp-intg-ws-us-east-2`
...
- **Region**: `us-east-2`
- **Status**: **Assigned to integration workspace** (created in staging)
...
|---------|------|---------------|---------|--------------|
| cl1 | `dp-intg-cluster-a` | i3.xlarge | 2 | 13.3.x-scala2.12 |
| cl2 | `dp-intg-cluster-b` | i3.2xlarge | 3 | 13.3.x-scala2.12 |
| cl3 | `dp-intg-cluster-c` | m5.xlarge | 4 | 13.3.x-scala2.12 |

### 5. **Unity Catalog Catalogs** (3 catalogs)

| Catalog | Storage Root | Grants | Metastore |
|---------|--------------|--------|-----------|
| intg_catalog1 | `s3://damage-prevention-dashboard-intg-us-east-2/catalogs/intg/catalog1/` | None | `dp-stag-metastore-us-east-2` ⭐ |
| intg_catalog2 | `s3://damage-prevention-dashboard-intg-us-east-2/catalogs/intg/catalog2/` | data-engineers (USE_CATALOG) | `dp-stag-metastore-us-east-2` ⭐ |
| intg_catalog3 | `s3://damage-prevention-dashboard-intg-us-east-2/catalogs/intg/catalog3/` | data-engineers (USE_CATALOG) | `dp-stag-metastore-us-east-2` ⭐ |
...
- **Name**: `dp-damage-prevention-intg-us-east-2-databricks-workspace`
- **Contents**:
...
```bash
cd ub-tf-dbx-envs/terraform/intg
...
# Note: service and environment are auto-computed (service="databricks", environment="intg")
```

#### Databricks Account / MWS
```hcl
databricks_account_id          = "1234567890123456"   # Your Databricks account ID
mws_credentials_id             = "cred-intg-001"      # Your MWS credentials ID
mws_storage_config_id          = "storage-intg-001"  # Your MWS storage config ID
mws_network_id                 = "network-intg-001"  # Your MWS network ID
mws_private_access_settings_id = "pas-intg-001"      # Your MWS PAS ID
```
...
workspace = {
  ...
  clusters = {
    cl1 = {
      cluster_name  = "dp-intg-cluster-a"  # Cluster name
  ...
  # 3 Catalogs (unique names to avoid conflicts with staging)
  catalogs = {
    intg_catalog1 = {
...
# - workspace_name = "{product_name}-intg-ws-{region}"
# - uc_metastore_name = "{product_name}-stag-metastore-{region}" (references staging)
# - uc_metastore_region = {region}
# - uc_external_prefix = "s3://{product_name}-dashboard-intg-{region}/unity-catalog/"
# - catalog storage_root = "s3://{product_name}-dashboard-intg-{region}/catalogs/intg/{catalog_name}/"
```
...
terraform {
  backend "s3" {
    ...
    key            = "dbx-envs/integration/terraform.tfstate"
    region         = "us-east-2"
    encrypt        = true
  }
}
...
**IMPORTANT**: Integration uses staging metastore. Deploy staging first:
...
Update integration `terraform.tfvars` with the staging metastore ID.
...
s3_bucket_name = "dp-damage-prevention-intg-us-east-2-s3"
```
...
catalog_names = {
  "intg_catalog1" = "intg_catalog1"
  "intg_catalog2" = "intg_catalog2"
  "intg_catalog3" = "intg_catalog3"
}
```
...
|--------|-------------|---------|
| `s3_bucket_name` | S3 bucket for Databricks | `dp-damage-prevention-intg-us-east-2-s3` |
...
| `catalog_names` | Map of catalog names | `{ "intg_catalog1" = "intg_catalog1", ... }` |

---

## Shared Metastore

The integration workspace accesses the metastore `dp-stag-metastore-us-east-2`, which is:
- **Created** in staging environment during Pass-1
- **Assigned** to the integration workspace (`dp-intg-ws-us-east-2`)
- **Used by** all 3 catalogs in this workspace
- **Shared** with staging workspace (both environments use the same metastore)
...
- **Status**: Created in staging, assigned to integration

### Catalog Naming

Since both environments share the same metastore, catalog names must be unique:
...
- **Solution**: 
  - Use unique catalog names (intg_catalog1, intg_catalog2, intg_catalog3)
  - Verify staging catalogs don't have the same names
...
- **Solution**: 
  ...
  - Ensure integration workspace is created first (Pass-1)
...
- **Examples**: `examples/intg/terraform.tfvars.example`
- **Deployment Guide**: `examples/intg/EXAMPLE_DEPLOYMENT.md`
