...
########################################
# Corporate tags (validated inside Repo 1)
########################################
...
variable "workspace" {
  ...
  type = object({
    ...
    # Unity Catalog
    uc_metastore_name   = optional(string)  # Auto-computed to reference staging if not provided
...
variable "shared_metastore_id" {
  type        = string
  description = "ID of existing Unity Catalog metastore to assign to workspace (from staging environment). If provided, metastore will not be created, only assigned."
  default     = null
}

variable "create_metastore" {
  type        = bool
  description = "Whether to create a new metastore (true) or use existing shared_metastore_id (false). Default: false (use shared metastore from staging)."
  default     = false
}
